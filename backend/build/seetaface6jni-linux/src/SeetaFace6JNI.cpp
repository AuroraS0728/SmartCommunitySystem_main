...
const int MASK_NO = 0;
const int MASK_YES = 1;
const int MASK_UNKNOWN = 2;
const int EYE_UNKNOWN = 3;
...
seeta::MaskDetector* MD; //口罩检测模块
seeta::EyeStateDetector* ESD; //眼睛状态检测模块
seeta::QualityAssessor* QA; //图片质量评估模块
...
static void release_image(SeetaImageData* image) {
	if (image) {
		if (image->data) {
			delete[] image->data;
			image->data = nullptr;
...
static bool detect_first_face(SeetaImageData* image, SeetaRect* face, vector<SeetaPointF>* points) {
	if (!image || !face || !points || !FD || !FL) {
		return false;
	}
...
(JNIEnv *env, jobject obj, jstring model_dir) {
	...
	string mask_model_path = dir + "mask_detector.csta";
	string eye_state_model_path = dir + "eye_state.csta";
	string quality_lbn_model_path = dir + "quality_lbn.csta";
	...
	if (file_exists(fas_first_model_path) && file_exists(fas_second_model_path)) {
		try {
			...
			LOGE("Initialize face antiSpoofing failed: %s\n", e.what());
			FAS = nullptr;
		}
	} else {
		LOGE("Skip face antiSpoofing model, model file not found.\n");
		FAS = nullptr;
	}

	if (file_exists(mask_model_path)) {
		try {
			LOGD("Initialize mask detector model: %s\n", mask_model_path.c_str());
			seeta::ModelSetting mask_setting;
			mask_setting.append(mask_model_path);
			MD = new seeta::MaskDetector(mask_setting);
		} catch (const std::exception& e) {
			LOGE("Initialize mask detector failed: %s\n", e.what());
			MD = nullptr;
		}
	}

	if (file_exists(eye_state_model_path)) {
		try {
			LOGD("Initialize eye state detector model: %s\n", eye_state_model_path.c_str());
			seeta::ModelSetting eye_setting;
			eye_setting.append(eye_state_model_path);
			ESD = new seeta::EyeStateDetector(eye_setting);
		} catch (const std::exception& e) {
			LOGE("Initialize eye state detector failed: %s\n", e.what());
			ESD = nullptr;
		}
	}

	try {
		LOGD("Initialize quality assessor.\n");
		QA = new seeta::QualityAssessor();
		QA->add_rule(seeta::BRIGHTNESS);
		if (file_exists(quality_lbn_model_path)) {
			seeta::ModelSetting quality_lbn_setting;
			quality_lbn_setting.append(quality_lbn_model_path);
			QA->add_rule(seeta::CLARITY, quality_lbn_setting);
		} else {
			QA->add_rule(seeta::CLARITY);
		}
		QA->add_rule(seeta::INTEGRITY);
		QA->add_rule(seeta::RESOLUTION);
		QA->add_rule(seeta::POSE_EX);
		QA->set_medium_limit(1);
	} catch (const std::exception& e) {
		LOGE("Initialize quality assessor failed: %s\n", e.what());
		QA = nullptr;
	}
...
(JNIEnv *env, jobject obj, jobject jimg) {
	...
	if (!FAS) {
		LOGE("face antiSpoofing model not initialized.\n");
		return FUZZY;
	}
...
(JNIEnv *env, jobject obj, jobject jimg) {
	...
	try {
		with_mask = MD->detect(*image, faces.data->pos, &score);
	} catch (const std::exception& e) {
		LOGE("detect mask failed: %s\n", e.what());
		release_image(image);
		return MASK_UNKNOWN;
	}
	release_image(image);
	return with_mask ? MASK_YES : MASK_NO;
}

JNIEXPORT jintArray JNICALL Java_com_seetaface_SeetaFace6JNI_detectEyeState
(JNIEnv *env, jobject obj, jobject jimg) {
	jint result[2] = {EYE_UNKNOWN, EYE_UNKNOWN};
	jintArray array = env->NewIntArray(2);
	if (!is_initialized || !ESD) {
	...
	if (!detect_first_face(image, &face, &points)) {
		release_image(image);
		env->SetIntArrayRegion(array, 0, 2, result);
		return array;
	}
	...
	try {
		...
		result[0] = static_cast<jint>(left);
		result[1] = static_cast<jint>(right);
	} catch (const std::exception& e) {
		LOGE("detect eye state failed: %s\n", e.what());
	}
	points.clear();
	release_image(image);
	env->SetIntArrayRegion(array, 0, 2, result);
...
(JNIEnv *env, jobject obj, jobject jimg) {
	jfloat result[3] = {0.0f, 0.0f, static_cast<jfloat>(seeta::LOW)};
	jfloatArray array = env->NewFloatArray(3);
	if (!is_initialized || !QA) {
		env->SetFloatArrayRegion(array, 0, 3, result);
		return array;
	}
	SeetaImageData* image = JniUtils::toSeetaImageData(env, jimg);
	SeetaRect face;
	...
	try {
		QA->feed(*image, face, points.data(), landmark_num);
		const int attrs[] = {seeta::BRIGHTNESS, seeta::CLARITY, seeta::INTEGRITY, seeta::RESOLUTION, seeta::POSE_EX};
		float score_sum = 0.0f;
		int score_count = 0;
		int min_level = seeta::HIGH;
		int medium_count = 0;
		bool passed = true;
		for (int attr : attrs) {
			seeta::QualityResult qr = QA->query(attr);
			score_sum += qr.score;
			score_count++;
			int level = static_cast<int>(qr.level);
			if (level < min_level) {
				min_level = level;
			}
			if (level == seeta::LOW) {
				passed = false;
			} else if (level == seeta::MEDIUM) {
				medium_count++;
			}
		}
		if (medium_count > 1) {
		...
		result[0] = passed ? 1.0f : 0.0f;
		result[1] = score_count == 0 ? 0.0f : score_sum / score_count;
		result[2] = static_cast<jfloat>(min_level);
	} catch (const std::exception& e) {
		LOGE("evaluate quality failed: %s\n", e.what());
	}
	points.clear();
	release_image(image);
	env->SetFloatArrayRegion(array, 0, 3, result);
