    ...
    <section class="panel">
      <div class="panel-header">
        ...
        <div class="header-actions">
          <el-input v-model="keyword" placeholder="搜索姓名、手机号、邀请码、入院事由" clearable class="search-input" />
      ...
      <el-table :data="displayList" stripe v-loading="loading">
        ...
        <el-table-column prop="visitReason" label="入院事由" min-width="180" show-overflow-tooltip />
...
const displayList = computed(() => {
  ...
  return records.value.filter((item) => {
    ...
    return (
      ...
      String(item.visitReason || '').includes(key)
    )
  })
})
...
async function removeBlacklistItem(row) {
  try {
    await ElMessageBox.confirm(`确认移除黑名单手机号 ${row.phone} 吗？`, '提示', {
      type: 'warning'
    })
